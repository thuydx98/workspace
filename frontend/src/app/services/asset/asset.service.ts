...
import { AddAssetModal, AssetModel } from 'src/app/models/asset/asset.model';
...
export class AssetService {
  ...
  public getPagingList(page: number = 1, size: number = 10): Observable<PagingListModel<AssetModel>> {
    return this.http.get(ApiRoutes.WorkSpaceApi.Assets + `?page=${page}&size=${size}`)
      .pipe(
  ...
  public addAsset(asset: AddAssetModal): Observable<AssetModel> {
    return this.http.post(ApiRoutes.WorkSpaceApi.Assets, asset)
      .pipe(
        map((res: any) => res.result)
      );
  }
